...
function populateOptions(options) {
    options = conflux_1.ethers.utils.shallowCopy(options || {});
...
function _compile(_solc, source, options) {
    ...
    for (var filename in output.contracts) {
        for (var name_1 in output.contracts[filename]) {
            ...
            result.push({
                name: name_1,
                interface: new conflux_1.ethers.utils.Interface(contract.abi),
...
// and for solc will fallback onto the version included in @confluxproject/cli
