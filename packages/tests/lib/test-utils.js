...
describe('Test Contract Address Generation', function () {
    ...
    var getContractAddress = conflux_1.ethers.utils.getContractAddress;
    var Tests = [
...
describe('Test RLP Coder', function () {
    ...
    tests.forEach(function (test) {
        it(('RLP coder encoded - ' + test.name), function () {
            this.timeout(120000);
            assert_1.default.equal(conflux_1.ethers.utils.RLP.encode(test.decoded), test.encoded, 'RLP encoded - ' + test.name);
        });
    });
    tests.forEach(function (test) {
        it(('RLP coder decoded - ' + test.name), function () {
            this.timeout(120000);
            assert_1.default.ok(equals(conflux_1.ethers.utils.RLP.decode(test.encoded), test.decoded), 'RLP decoded - ' + test.name);
        });
    });
});
describe('Test Unit Conversion', function () {
    ...
    tests.forEach(function (test) {
        var wei = conflux_1.ethers.BigNumber.from(test.wei);
        it(('parses ' + test.ether + ' ether'), function () {
            assert_1.default.ok(conflux_1.ethers.utils.parseEther(test.ether.replace(/,/g, '')).eq(wei), 'parsing ether failed - ' + test.name);
        });
        it(('formats ' + wei.toString() + ' wei to ether'), function () {
            var actual = conflux_1.ethers.utils.formatEther(wei);
        ...
        ['kwei', 'mwei', 'gwei', 'szabo', 'finney', 'satoshi'].forEach(function (name) {
            ...
            if (test[name]) {
                it(('parses ' + test[name] + ' ' + name), function () {
                    this.timeout(120000);
                    assert_1.default.ok(conflux_1.ethers.utils.parseUnits(test[name].replace(/,/g, ''), unitName).eq(wei), ('parsing ' + name + ' failed - ' + test.name));
                });
            }
            var expectedKey = (name + '_format');
            if (test[expectedKey]) {
                it(('formats ' + wei.toString() + ' wei to ' + name + ')'), function () {
                    var actual = conflux_1.ethers.utils.formatUnits(wei, unitName);
    ...
    it("formats with commify", function () {
        ...
        Object.keys(tests).forEach(function (test) {
            assert_1.default.equal(conflux_1.ethers.utils.commify(test), tests[test]);
        });
    });
});
describe('Test Namehash', function () {
    ...
    tests.forEach(function (test) {
        it(('computes namehash - "' + test.name + '"'), function () {
            this.timeout(120000);
            assert_1.default.equal(conflux_1.ethers.utils.namehash(test.name), test.expected, 'computes namehash(' + test.name + ')');
        });
    });
    it("isValidName", function () {
        assert_1.default.ok(conflux_1.ethers.utils.isValidName("ricmoo.eth"));
        assert_1.default.ok(!conflux_1.ethers.utils.isValidName(""));
        assert_1.default.ok(!conflux_1.ethers.utils.isValidName("ricmoo..eth"));
    });
});
describe('Test ID Hash Functions', function () {
    ...
    tests.forEach(function (test) {
        it(('computes id - ' + test.name), function () {
            this.timeout(120000);
            var actual = conflux_1.ethers.utils.id(test.text);
    ...
    function test(funcName, testKey) {
        it("computes " + funcName + " correctly", function () {
            ...
            tests.forEach(function (test, index) {
                var actual = (conflux_1.ethers.utils)['solidity' + funcName](test.types, test.values);
    ...
    testsInvalid.forEach(function (type) {
        it("disallows invalid type \"" + type + "\"", function () {
            assert_1.default.throws(function () {
                conflux_1.ethers.utils.solidityPack([type], ["0x12"]);
            }, function (error) {
...
describe('Test Hash Functions', function () {
    ...
    it('computes keccak256 correctly', function () {
        this.timeout(120000);
        tests.forEach(function (test) {
            assert_1.default.equal(conflux_1.ethers.utils.keccak256(test.data), test.keccak256, ('Keccak256 - ' + test.data));
        });
    });
    it('computes sha2-256 correctly', function () {
        this.timeout(120000);
        tests.forEach(function (test) {
            assert_1.default.equal(conflux_1.ethers.utils.sha256(test.data), test.sha256, ('SHA256 - ' + test.data));
        });
    });
    it('computes sha2-512 correctly', function () {
        this.timeout(120000);
        tests.forEach(function (test) {
            assert_1.default.equal(conflux_1.ethers.utils.sha512(test.data), test.sha512, ('SHA512 - ' + test.data));
        });
    });
});
describe('Test Solidity splitSignature', function () {
    it('splits a canonical signature', function () {
        ...
        for (var v = 27; v <= 28; v++) {
            var signature = conflux_1.ethers.utils.concat([r, s, [v]]);
            var sig = conflux_1.ethers.utils.splitSignature(signature);
    ...
    it('splits a legacy signature', function () {
        ...
        var s = '0xcafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7e';
        for (var v = 27; v <= 28; v++) {
            var signature = conflux_1.ethers.utils.concat([r, s, [v - 27]]);
            var sig = conflux_1.ethers.utils.splitSignature(signature);
            assert_1.default.equal(sig.r, r, 'split r correctly');
            assert_1.default.equal(sig.s, s, 'split s correctly');
            assert_1.default.equal(sig.v, v, 'split v correctly');
...
describe('Test Base64 coder', function () {
    ...
    it('encodes and decodes the example from wikipedia', function () {
        ...
        var decoded = conflux_1.ethers.utils.toUtf8Bytes(decodedText);
        ...
        assert_1.default.equal(conflux_1.ethers.utils.base64.encode(decoded), encoded, 'encodes to base64 string');
        assert_1.default.equal(conflux_1.ethers.utils.toUtf8String(conflux_1.ethers.utils.base64.decode(encoded)), decodedText, 'decodes from base64 string');
    });
});
describe('Test UTF-8 coder', function () {
    var overlong = conflux_1.ethers.utils.Utf8ErrorReason.OVERLONG;
    var utf16Surrogate = conflux_1.ethers.utils.Utf8ErrorReason.UTF16_SURROGATE;
    var overrun = conflux_1.ethers.utils.Utf8ErrorReason.OVERRUN;
    var missingContinue = conflux_1.ethers.utils.Utf8ErrorReason.MISSING_CONTINUE;
    var unexpectedContinue = conflux_1.ethers.utils.Utf8ErrorReason.UNEXPECTED_CONTINUE;
    var outOfRange = conflux_1.ethers.utils.Utf8ErrorReason.OUT_OF_RANGE;
    var BadUTF = [
    ...
    BadUTF.forEach(function (test) {
        it('toUtf8String - ' + test.name, function () {
            ...
            var ignored = conflux_1.ethers.utils.toUtf8String(test.bytes, conflux_1.ethers.utils.Utf8ErrorFuncs.ignore);
            assert_1.default.equal(ignored, test.ignored, "ignoring errors matches");
            // Check the string using the replaceErrors conversion
            var replaced = conflux_1.ethers.utils.toUtf8String(test.bytes, conflux_1.ethers.utils.Utf8ErrorFuncs.replace);
            ...
            assert_1.default.throws(function () {
                var result = conflux_1.ethers.utils.toUtf8String(test.bytes);
    ...
    it('toUtf8String - random conversions', function () {
        ...
        for (var i = 0; i < 100000; i++) {
            ...
            var bytes = conflux_1.ethers.utils.toUtf8Bytes(str);
            var str2 = conflux_1.ethers.utils.toUtf8String(bytes);
            var escaped = JSON.parse(conflux_1.ethers.utils._toEscapedUtf8String(bytes));
...
describe('Test Bytes32String coder', function () {
    ...
    it("encodes an ens name", function () {
        ...
        var bytes32 = conflux_1.ethers.utils.formatBytes32String(str);
        var str2 = conflux_1.ethers.utils.parseBytes32String(bytes32);
...
function getHex(value) {
    return conflux_1.ethers.utils.hexlify(conflux_1.ethers.utils.toUtf8Bytes(value));
}
describe("Test nameprep", function () {
    ...
    Tests.forEach(function (test) {
        ...
        it(test.comment, function () {
            var input = conflux_1.ethers.utils.toUtf8String(test.input);
            if (test.output) {
                var expected = conflux_1.ethers.utils.toUtf8String(test.output);
                var actual = conflux_1.ethers.utils.nameprep(input);
...
describe("Test Signature Manipulation", function () {
    ...
    tests.forEach(function (test) {
        it("autofills partial signatures - " + test.name, function () {
            var address = conflux_1.ethers.utils.getAddress(test.accountAddress);
            var hash = conflux_1.ethers.utils.keccak256(test.unsignedTransaction);
            var data = conflux_1.ethers.utils.RLP.decode(test.signedTransaction);
            var s = data.pop(), r = data.pop(), v = parseInt(data.pop().substring(2), 16);
            var sig = conflux_1.ethers.utils.splitSignature({ r: r, s: s, v: v });
            {
                var addr = conflux_1.ethers.utils.recoverAddress(hash, {
                    r: r, s: s, v: v
                });
...
describe("BigNumber", function () {
    ...
    tests.forEach(function (test) {
        if (test.expectedValue == null) {
            it(test.testcase, function () {
                assert_1.default.throws(function () {
                    var value = conflux_1.ethers.BigNumber.from(test.value);
            ...
            it(test.testcase, function () {
                ...
                var value2 = conflux_1.ethers.BigNumber.from(value);
    ...
    [
        ...
        it("absolute value (" + test.value + ")", function () {
            var value = conflux_1.ethers.BigNumber.from(test.value);
            var expected = conflux_1.ethers.BigNumber.from(test.expected);
    ...
    it("Fails on junk with a length property", function () {
        ...
        assert_1.default.throws(function () {
            var value = conflux_1.ethers.BigNumber.from("100").add(junk);
            console.log("ERROR", value);
        }, function (error) {
            return true;
        });
...
describe("FixedNumber", function () {
    {
        ...
        Tests.forEach(function (test) {
            it("Create from=" + test.value, function () {
                var value = conflux_1.ethers.FixedNumber.from(test.value);
    ...
    }
    {
        ...
        Tests.forEach(function (test) {
            it("Rounding value=" + test.value + ", decimals=" + test.round, function () {
                var value = conflux_1.ethers.FixedNumber.from(test.value).round(test.round);
                assert_1.default.equal(value.toString(), test.expected);
            });
        });
    }
...
describe("Logger", function () {
    var logger = new conflux_1.ethers.utils.Logger("testing/0.0");
    it("setLogLevel", function () {
        conflux_1.ethers.utils.Logger.setLogLevel(conflux_1.ethers.utils.Logger.levels.DEBUG);
        conflux_1.ethers.utils.Logger.setLogLevel(conflux_1.ethers.utils.Logger.levels.INFO);
        conflux_1.ethers.utils.Logger.setLogLevel(conflux_1.ethers.utils.Logger.levels.WARNING);
        conflux_1.ethers.utils.Logger.setLogLevel(conflux_1.ethers.utils.Logger.levels.ERROR);
        conflux_1.ethers.utils.Logger.setLogLevel(conflux_1.ethers.utils.Logger.levels.OFF);
    ...
    it("checkArgumentCount - too few", function () {
        assert_1.default.throws(function () {
            ...
            return error.code === conflux_1.ethers.utils.Logger.errors.MISSING_ARGUMENT;
        });
    });
    it("checkArgumentCount - too many", function () {
        assert_1.default.throws(function () {
            ...
            return error.code === conflux_1.ethers.utils.Logger.errors.UNEXPECTED_ARGUMENT;
        });
    });
});
describe("Base58 Coder", function () {
    it("decodes", function () {
        assert_1.default.equal(conflux_1.ethers.utils.toUtf8String(conflux_1.ethers.utils.base58.decode("JxF12TrwUP45BMd")), "Hello World");
    });
    it("encodes", function () {
        assert_1.default.equal(conflux_1.ethers.utils.base58.encode(conflux_1.ethers.utils.toUtf8Bytes("Hello World")), "JxF12TrwUP45BMd");
    });
});
/*
...
describe("EIP-712", function () {
    ...
    tests.forEach(function (test) {
        it("encoding " + test.name, function () {
            var encoder = conflux_1.ethers.utils._TypedDataEncoder.from(test.types);
            ...
            //console.log(test);
            assert_1.default.equal(conflux_1.ethers.utils._TypedDataEncoder.getPrimaryType(test.types), test.primaryType, "getPrimaryType");
            assert_1.default.equal(conflux_1.ethers.utils._TypedDataEncoder.hash(test.domain, test.types, test.data), test.digest, "digest");
        });
    });
    tests.forEach(function (test) {
        if (!test.privateKey) {
        ...
        it("signing " + test.name, function () {
            return __awaiter(this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            wallet = new conflux_1.ethers.Wallet(test.privateKey);
